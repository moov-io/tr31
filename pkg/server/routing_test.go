...
func TestRouting_machine_mgmt(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "80cae8bed08fe2cc", response1.IK)
	...
	descriptData := decryptDataRequest{
		keyPath:  "/secret/tr31", // Fix typo "secrect" -> "secret"
		keyName:  "kbkp",
		keyBlock: "A0088M3TC00E000022BD7EC46BBE2A6A73389D1BA6DB63120B386F912839F4679C0523399E4D8D0F1D9A356E",
		timeout:  10,
	}

	requestBody, err = json.Marshal(descriptData)
	fmt.Printf("%s", requestBody)
	require.NoError(t, err)

	req = httptest.NewRequest("POST", "/decrypt_data/80cae8bed08fe2cc", bytes.NewReader(requestBody))
	req.Header.Set("Origin", "https://moov.io")
	...
	require.Equal(t, "80cae8bed08fe2cc", response4.Data)
}
